...
| `library_enrichment/destination_rebind_inspect` | `kind` (`mirror` or `playback`), `subscription_id`, `candidate_item_id` | compare an applied destination's saved raw M3U digest with a candidate builtin playlist after restore; no write |
| `library_enrichment/destination_rebind_apply` | `kind`, `subscription_id`, `candidate_item_id`, `expected_old_item_id`, `expected_content_digest`, `expected_observed_digest`, optional `expected_revision` for a mirror | re-read the candidate under its playlist lock and update the saved destination ID only if the bytes and checkpoint still match; also requires `library.write` |
